...
class InfoMap:
    ...
    def get_forecasts_df(self) -> pd.DataFrame:
        df = pd.DataFrame(columns=['datetime', 'temp', 'city', 'advice','humidity','pressure'])
        ...
        if messages:
            ...
            for f in messages[maxKey]['cities']:
                ...
                for w in f['weather']:
                    ...
                    df.loc[len(df), df.columns] = dt, temp, city, adv, humidity, pressure
        return df
    ...
    def plot_map(self) -> None:
        ...
        fig.update_traces(hovertemplate="<b>%{text} </b><br>" +
                          ...
                          "humidity: %{customdata[0]} °C<br>" +
                          "pressure: %{customdata[0]} °C<br>" +
